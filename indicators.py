...
# Qmoy/QA (mean discharge over a chosen period)
...
def Qmoy(df):
    ...
    Returns:
        df: Original dataframe with added new-date and mean discharge (Qmoy) stats.
    """
    print("Mean Discharge Calculation (Qmoy): available columns")
    for i, col in enumerate(df.columns):
        print(f" [{i}] {col}")
...
def VCN10(df):
    ...
    try:
        ...
        nb = int(input(f"Enter the calculation period in {label_unite} (e.g., '3' to find the minimum 10-day mean within every 3-{unite} block): "))
...
def Q10_05(df):
    ...
    try:
        ...
        nb = int(input(f"Enter the calculation period in {label_unite} (e.g., '1' for the high-flow quantiles every 1 {unite[:-1]}): "))
        frequence = f"{nb}{freq_map[unite]}"
    except ValueError as e:
        raise ValueError(f"Input error: {e}")
...
def VCX3(df):
    ...
    try:
        ...
        nb = int(input(f"Enter the calculation period in {label_unite} (e.g., '3' to find the maximum 3-day consecutive mean flow every 3 {unite[:-1]}): "))
